	 ...
	 FROM
		(
			SELECT
			   ...
			   oo.order_id,
			   oo.order_purchase_timestamp,
			   LAG(oo.order_purchase_timestamp, 1) OVER (PARTITION BY oi.seller_id
			       ORDER BY oo.order_purchase_timestamp) AS previous_order_date
			FROM
				olist_orders oo
			JOIN 
			    olist_items oi ON oi.order_id = oo.order_id
...
FROM olist_reviews ore
JOIN olist_orders oo ON oo.order_id = ore.order_id 
WHERE
	order_status <> 'canceled'
	AND order_delivered_customer_date IS NOT NULL
GROUP BY order_id
...
# item+payment+order:115027 rows	
SELECT 
opa.order_id,
opa.payment_type,
-- COUNT(oi.product_id) 
oi.product_id 
FROM olist_payments opa 
JOIN olist_items oi ON opa.order_id = oi.order_id 
JOIN olist_products op ON op.product_id = oi.product_id 
JOIN olist_orders oo ON opa.order_id =oo.order_id 
WHERE oo.order_status <> 'canceled' AND oo.order_delivered_customer_date IS NOT NULL
-- WHERE opa.order_id ='285c2e15bebd4ac83635ccc563dc71f4' 
-- GROUP BY opa.order_id,opa.payment_type 
-- HAVING COUNT(oi.product_id) >1
ORDER BY opa.order_id
...
FROM (
	...
	JOIN(
		...
		FROM
			olist_payments opa
		JOIN olist_orders oo ON oo.order_id = opa.order_id 
		WHERE 	
			oo.order_status <> 'canceled'
			AND oo.order_delivered_customer_date IS NOT NULL
		GROUP BY
 ...
 */
	...
	WHERE ROUND((payment_value - cost), 0) > 0
	ORDER BY ROUND((payment_value - cost), 0) > 0 DESC; -- 233 rows
...
/*14. How does Olist's marketing spend and channel mix impact sales and customer acquisition costs, 
 *    and how can the company optimize its marketing strategy to increase ROI?
 */
